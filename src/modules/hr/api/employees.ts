...
export interface EmployeeSummary {
  ...
  departmentName: string | null; jobTitleName: string | null; payGradeName: string | null; startDate: string | null; endDate: string | null;
  nationalId?: string | null; personalEmail?: string | null; gender?: string | null; dateOfBirth?: string | null;
  departmentId?: string | null; jobTitleId?: string | null; payGradeId?: string | null; employmentType?: EmployeeFormValues['employmentType']; contractType?: EmployeeFormValues['contractType']; contractEndDate?: string | null;
  ...
  paymentMethod?: EmployeeFormValues['paymentMethod']; mobileMoneyNumber?: string | null; bankName?: string | null; accountNumber?: string | null; sortCode?: string | null; tinNumber?: string | null; nssfNumber?: string | null;
  employeeTaxType?: EmployeeFormValues['employeeTaxType']; pctMonthWorked?: number; whtRate?: number;
}
export interface EmployeeSetup { departments: EmployeeSetupOption[]; jobTitles: EmployeeSetupOption[]; payGrades: EmployeeSetupOption[] }
...
type PeriodRow = { start_date: string; end_date: string | null; department_id: string | null; job_title_id: string | null; pay_grade_id: string | null; employment_type: EmployeeFormValues['employmentType']; contract_type: EmployeeFormValues['contractType']; probation_end_date: string | null; probation_status: EmployeeFormValues['probationStatus']; departments: { name: string } | null; job_titles: { name: string } | null; pay_grades: { name: string } | null }
...
const employeeSelection = `id, employee_number, legal_name, company_email, personal_email, work_phone, gender, date_of_birth, archived_at, employee_confidential_profiles(national_id,gross_salary,currency_code,custom_overtime_rate,payment_method,mobile_money_number,bank_name,account_number,sort_code,tin_number,nssf_number,employee_tax_type,pct_month_worked,wht_rate), employment_periods(start_date, end_date, department_id, job_title_id, pay_grade_id, employment_type, contract_type, probation_end_date, probation_status, departments(name), job_titles(name), pay_grades(name))`
...
function mapEmployee(row: EmployeeRow): EmployeeSummary {
  ...
  return { id: row.id, employeeNumber: row.employee_number, legalName: row.legal_name, companyEmail: row.company_email, workPhone: row.work_phone,
    active: !row.archived_at && Boolean(current && (!current.end_date || current.end_date >= today)), departmentName: current?.departments?.name ?? null, jobTitleName: current?.job_titles?.name ?? null, payGradeName: current?.pay_grades?.name ?? null, startDate: current?.start_date ?? null, endDate: current?.end_date ?? null,
    nationalId: optional(confidential.national_id), personalEmail: optional(row.personal_email), gender: optional(row.gender), dateOfBirth: optional(row.date_of_birth), departmentId: current?.department_id, jobTitleId: current?.job_title_id, payGradeId: current?.pay_grade_id,
    ...
    grossSalary: numeric(confidential.gross_salary), currency: 'UGX', customOvertimeRate: numeric(confidential.custom_overtime_rate), paymentMethod: (optional(confidential.payment_method) as EmployeeFormValues['paymentMethod']) ?? 'cash', mobileMoneyNumber: optional(confidential.mobile_money_number), bankName: optional(confidential.bank_name), accountNumber: optional(confidential.account_number), sortCode: optional(confidential.sort_code), tinNumber: optional(confidential.tin_number), nssfNumber: optional(confidential.nssf_number), employeeTaxType: (optional(confidential.employee_tax_type) as EmployeeFormValues['employeeTaxType']) ?? 'local', pctMonthWorked: numeric(confidential.pct_month_worked) ?? 100, whtRate: numeric(confidential.wht_rate) ?? 6 }
}
