...
export function EmployeeDossierPage({ employeeId, api = employeeApi }: { employeeId: string; api?: EmployeeApi }) {
  ...
  return <section className="oh-workspace-page">
    ...
    <div className="oh-dossier-grid">
      ...
      <article className="oh-info-card"><h2>Salary & payment</h2><dl><div><dt>Gross monthly salary</dt><dd>{record.grossSalary == null ? 'Not provided' : `UGX ${record.grossSalary.toLocaleString()}`}</dd></div><div><dt>Mobile money</dt><dd>{record.mobileMoneyNumber ?? 'Not provided'}</dd></div><div><dt>Bank</dt><dd>{record.bankName ?? 'Not provided'}</dd></div><div><dt>Account number</dt><dd>{record.accountNumber ?? 'Not provided'}</dd></div></dl></article>
