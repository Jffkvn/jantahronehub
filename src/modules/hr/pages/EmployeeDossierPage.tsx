...
import { leaveApi, type LeaveApi } from '../api/leave'
...
export function EmployeeDossierPage({ employeeId, api = employeeApi, leave = leaveApi, permissions = [] }: { employeeId: string; api?: EmployeeApi; leave?: LeaveApi; permissions?: string[] }) {
  ...
  const canManageLeave = permissions.includes('leave.manage')
  const leaveBalances = useQuery({ queryKey: ['leave-balances', employeeId, new Date().getFullYear()], queryFn: () => leave.listBalances(employeeId, new Date().getFullYear()), enabled: canManageLeave })
  const leaveRequests = useQuery({ queryKey: ['hr-leave-requests'], queryFn: leave.listForHr, enabled: canManageLeave })
  ...
  return <section className="oh-workspace-page">
    ...
    {canManageLeave ? <nav className="oh-inline-actions" aria-label="Employee dossier sections"><Button variant={section === 'profile' ? 'primary' : 'secondary'} onClick={() => setSection('profile')}>Profile</Button><Button variant={section === 'leave' ? 'primary' : 'secondary'} onClick={() => setSection('leave')}>Leave</Button></nav> : null}
    {section === 'profile' ? <div className="oh-dossier-grid">
      ...
      <article className="oh-info-card"><h2>Current employment</h2><dl><div><dt>Start date</dt><dd>{record.startDate ?? 'Not recorded'}</dd></div><div><dt>Pay grade</dt><dd>{record.payGradeName ?? 'Not assigned'}</dd></div><div><dt>Contract type</dt><dd>{record.contractType?.replace('_', ' ') ?? 'Not recorded'}</dd></div><div><dt>Contract / last day</dt><dd>{record.endDate ?? 'Open-ended'}</dd></div><div><dt>Probation</dt><dd>{record.probationStatus?.replace('_', ' ') ?? 'Not applicable'}</dd></div></dl></article>
    ...
    </div> : <section className="oh-form-stack"><header><h2>Leave record</h2><p>{new Date().getFullYear()} balances and complete request history.</p></header>{leaveBalances.isLoading || leaveRequests.isLoading ? <p role="status">Loading employee leave…</p> : null}<LeaveBalanceCards balances={leaveBalances.data ?? []} /><section className="oh-card"><h2>Leave history</h2>{leaveRequests.data?.filter((request) => request.employeeId === employeeId).length ? <div className="oh-table-wrap"><table className="oh-table"><thead><tr><th>Leave</th><th>Dates</th><th>Days</th><th>Status</th></tr></thead><tbody>{leaveRequests.data.filter((request) => request.employeeId === employeeId).map((request) => <tr key={request.id}><td>{request.leaveTypeName}</td><td>{request.startDate} to {request.endDate}</td><td>{request.workingDays}</td><td><LeaveStatusBadge status={request.status} /></td></tr>)}</tbody></table></div> : <p>No leave requests recorded for this employee.</p>}</section></section>}
    <Modal open={editing} title="Edit employee" onClose={closeEditing}><EmployeeForm departments={setup.data?.departments} jobTitles={setup.data?.jobTitles} payGrades={setup.data?.payGrades} submitting={update.isPending} initialValues={{ fullName: record.legalName, nationalId: record.nationalId ?? '', companyEmail: record.companyEmail ?? '', personalEmail: record.personalEmail ?? '', phone: record.workPhone ?? '', gender: (record.gender as EmployeeFormValues['gender']) ?? '', dateOfBirth: record.dateOfBirth ?? '', departmentId: record.departmentId ?? '', jobTitleId: record.jobTitleId ?? '', payGradeId: record.payGradeId ?? '', startDate: record.startDate ?? '', employmentType: record.employmentType ?? 'full_time', contractType: record.contractType ?? 'permanent', contractEndDate: record.contractEndDate ?? '', probationEndDate: record.probationEndDate ?? '', probationStatus: record.probationStatus ?? 'not_applicable', grossSalary: record.grossSalary?.toString() ?? '', currency: 'UGX', customOvertimeRate: record.customOvertimeRate?.toString() ?? '', paymentMethod: record.paymentMethod ?? 'cash', mobileMoneyNumber: record.mobileMoneyNumber ?? '', bankName: record.bankName ?? '', accountNumber: record.accountNumber ?? '', sortCode: record.sortCode ?? '', employeeNumber: record.employeeNumber, tinNumber: record.tinNumber ?? '', nssfNumber: record.nssfNumber ?? '', employeeTaxType: record.employeeTaxType ?? 'local', pctMonthWorked: record.pctMonthWorked?.toString() ?? '100', whtRate: record.whtRate?.toString() ?? '6' }} onSubmit={async (values) => { await update.mutateAsync(values) }} /></Modal>
