...
export function EmployeeDirectoryPage({ api = employeeApi }: { api?: EmployeeApi }) {
  ...
  return <section className="oh-workspace-page">
    <header className="oh-page-header"><div><p>People operations</p><h1>Employee directory</h1><span>Maintain employment records, assignments and offboarding in one place.</span></div><div className="oh-dossier-actions"><Link className="oh-button oh-button--secondary" to="/hr/employees/import"><FileSpreadsheet size={17} /> Import / export</Link><Button onClick={() => setCreating(true)}><UserPlus size={17} /> Add employee</Button></div></header>
